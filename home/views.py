...
class HomeView(View):
	...
	def get(self, request):
		...
		sort_by, filter_by_price, filter_by_color, filter_by_tags = get_request_params(request)
		product_qs = Product.objects.filter(stock__gt=0)
		category_qs = Category.objects.all()
		
		if sort_by:
			if sort_by == "average_rating":
				product_qs = product_qs.annotate(average_rating=Avg('reviews__stars')).order_by('-average_rating')
		...
		if filter_by_color:
			product_qs = product_qs.filter(product_size_and_color__color__name=filter_by_color)
		...
		if request.user.is_authenticated:
			product_qs = product_qs.annotate(is_in_my_wishlist=ExpressionWrapper(Q(wish_list__user__id=request.user.id), output_field=BooleanField()))

		if not request.user.is_authenticated:
			fav_products_list = request.session.get("fav_products", [])

			if len(fav_products_list) > 0:
				product_qs = product_qs.annotate(is_in_my_wishlist=ExpressionWrapper(Q(id__in=fav_products_list), output_field=BooleanField()))
